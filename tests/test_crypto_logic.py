...
class TestCryptoLogic(unittest.TestCase):
    ...
    def test_derive_key_success(self):
        ...
        key = crypto_logic.derive_key(self.password, self.salt)
        self.assertEqual(len(key), crypto_logic.KEY_SIZE)
        self.assertIsInstance(key, bytes)

        # Same password and salt should produce the same key
        key2 = crypto_logic.derive_key(self.password, self.salt)
        self.assertEqual(key, key2)

        # Different salt should produce a different key
        different_salt = os.urandom(crypto_logic.SALT_SIZE)
        key3 = crypto_logic.derive_key(self.password, different_salt)
        self.assertNotEqual(key, key3)

    def test_derive_key_invalid_input(self):
        """Test key derivation with invalid inputs."""
        with self.assertRaises(ValueError):
            crypto_logic.derive_key("", self.salt)  # Empty password
        with self.assertRaises(ValueError):
            crypto_logic.derive_key(self.password, b'')  # Empty salt
        with self.assertRaises(ValueError):
            crypto_logic.derive_key(self.password, os.urandom(8)) # Invalid salt size
    ...
    def test_decrypt_wrong_key(self):
        ...
        key1 = crypto_logic.derive_key(self.password, self.salt)
        nonce, ciphertext = crypto_logic.encrypt(self.plain_data, key1)

        wrong_password = "wrong-password"
        wrong_key = crypto_logic.derive_key(wrong_password, self.salt)
